...
Sui の Cetus Protocol は共有の `integer-mate` ライブラリ内の欠陥のある `checked_shlw` を介して悪用されました。この関数は CLMM の流動性計算時の 64 ビットによる左シフトの際に、u256 のオーバーフローを防ぐことを意図していました。このオーバーフローチェックは誤った閾値 (`1 << 192` ではなく `0xFFFFFFFFFFFFFFFF << 192`) を使用しており、2^192 以上の値を通過してしまいました。Move では、左シフトはオーバーフローで中断せず、切り捨てとなります。切り捨てられた数値によって、`get_delta_a` は、膨大な流動性を生成するために必要となるのは 1 トークンのみであると返すことになりました。攻撃者はフラッシュスワップを用いてプール全体にわたってこれを繰り返し、約 2 億 2300 万ドルを流出しました。主な教訓: (1) Move のシフト演算はオーバーフローで中断しない。(2) カスタムオーバーフローガードは厳密に検証されなければならない。(3) 共有の数学ライブラリは高リスクであり、形式的解析が必要である。
